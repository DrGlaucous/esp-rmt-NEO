...
#include "driver/periph_ctrl.h"//"esp_private/periph_ctrl.h"
...
rmt_group_t *rmt_acquire_group_handle(int group_id)
{
    ...
    if (new_group) {
        //PRIx32
        ESP_LOGD(TAG, "new group(%d) at %p, occupy=%d", group_id, group, group->occupy_mask);
    }
    return group;
}
...
esp_err_t rmt_select_periph_clock(rmt_channel_handle_t chan, rmt_clock_source_t clk_src)
{
    ...
    rmt_ll_set_group_clock_src((rmt_dev_t*)group->hal.regs, channel_id, clk_src, 1, 1, 0); //ALTER (cast)
    group->resolution_hz = periph_src_clk_hz;
    //PRIx32
    ESP_LOGD(TAG, "group clock resolution:%d", group->resolution_hz);
    return ret;
}
