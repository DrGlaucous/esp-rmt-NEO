...
#include "esp_log.h"
#include "esp_check.h"
//#include "esp_memory_utils.h"//ALTER
...
esp_err_t rmt_new_rx_channel(const rmt_rx_channel_config_t *config, rmt_channel_handle_t *ret_chan)
{
    ...
    rmt_ll_rx_set_channel_clock_div((rmt_dev_t*)hal->regs, channel_id, real_div); //ALTER (cast)
    ...
    if (rx_channel->base.resolution_hz != config->resolution_hz) {
        ...
        ESP_LOGW(TAG, "channel resolution loss, real=%d", rx_channel->base.resolution_hz);
    }

    rmt_ll_rx_set_mem_blocks((rmt_dev_t*)hal->regs, channel_id, rx_channel->base.mem_block_num); //ALTER (cast)
    rmt_ll_rx_set_mem_owner((rmt_dev_t*)hal->regs, channel_id, RMT_LL_MEM_OWNER_HW); //ALTER (cast)
    ...
    rx_channel->base.hw_mem_base = (rmt_symbol_word_t*)&RMTMEM.chan[channel_id + RMT_RX_CHANNEL_OFFSET_IN_GROUP].data32[0]; //ALTER &RMTMEM_n.channels[channel_id + RMT_RX_CHANNEL_OFFSET_IN_GROUP].symbols[0];
    ...
    *ret_chan = &rx_channel->base;
    //PRIu32
    ESP_LOGD(TAG, "new rx channel(%d,%d) at %p, gpio=%d, res=%d Hz, hw_mem_base=%p, ping_pong_size=%d",
...
esp_err_t rmt_receive(rmt_channel_handle_t channel, void *buffer, size_t buffer_size, const rmt_receive_config_t *config)
{
    ...
    rmt_ll_rx_reset_pointer((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
    rmt_ll_rx_set_mem_owner((rmt_dev_t*)hal->regs, channel_id, RMT_LL_MEM_OWNER_HW); //ALTER (cast)
    // set sampling parameters of incoming signals
    rmt_ll_rx_set_filter_thres((rmt_dev_t*)hal->regs, channel_id, ((uint64_t)group->resolution_hz * config->signal_range_min_ns) / 1000000000UL); //ALTER (cast)
    rmt_ll_rx_enable_filter((rmt_dev_t*)hal->regs, channel_id, config->signal_range_min_ns != 0); //ALTER (cast)
    rmt_ll_rx_set_idle_thres((rmt_dev_t*)hal->regs, channel_id, ((uint64_t)channel->resolution_hz * config->signal_range_max_ns) / 1000000000UL); //ALTER (cast)
    // turn on RMT RX machine
    rmt_ll_rx_enable((rmt_dev_t*)hal->regs, channel_id, true); //ALTER (cast)
...
static esp_err_t rmt_rx_enable(rmt_channel_handle_t channel)
{
    ...
    if (channel->dma_chan) {
        ...
        portENTER_CRITICAL(&group->spinlock);
        rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_RX_MASK(channel_id), true); //ALTER (cast)
        portEXIT_CRITICAL(&group->spinlock);
    }
...
static esp_err_t rmt_rx_disable(rmt_channel_handle_t channel)
{
    ...
    if (channel->dma_chan) {
        ...
        portENTER_CRITICAL(&group->spinlock);
        rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_RX_MASK(channel_id), false); //ALTER (cast)
        rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_RX_MASK(channel_id)); //ALTER (cast)
        portEXIT_CRITICAL(&group->spinlock);
    }
...
static bool IRAM_ATTR rmt_isr_handle_rx_done(rmt_rx_channel_t *rx_chan)
{
    ...
    rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_RX_DONE(channel_id)); //ALTER (cast)
    ...
    if (rmt_ll_rx_get_interrupt_status_raw((rmt_dev_t*)hal->regs, channel_id) & RMT_LL_EVENT_RX_ERROR(channel_id)) { //ALTER (cast)
        portENTER_CRITICAL_ISR(&channel->spinlock);
        rmt_ll_rx_reset_pointer((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
        portEXIT_CRITICAL_ISR(&channel->spinlock);
        // this clear operation can only take effect after we copy out the received data and reset the pointer
        rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_RX_ERROR(channel_id)); //ALTER (cast)
...
static void IRAM_ATTR rmt_rx_default_isr(void *args)
{
    ...
    uint32_t status = rmt_ll_rx_get_interrupt_status((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
