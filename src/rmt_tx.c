...
esp_err_t rmt_new_tx_channel(const rmt_tx_channel_config_t *config, rmt_channel_handle_t *ret_chan)
{
    ...
    ret = esp_intr_alloc_intrstatus(rmt_periph_signals.groups[group_id].irq, isr_flags,
                                    (uint32_t)rmt_ll_get_interrupt_status_reg((rmt_dev_t*)hal->regs), //ALTER (cast)
    ...
    rmt_ll_tx_set_channel_clock_div((rmt_dev_t*)hal->regs, channel_id, real_div); //ALTER (cast)
    ...
    if (tx_channel->base.resolution_hz != config->resolution_hz) {
        ESP_LOGW(TAG, "channel resolution loss, real=%d", tx_channel->base.resolution_hz); //PRIu32
    }

    rmt_ll_tx_set_mem_blocks((rmt_dev_t*)hal->regs, channel_id, tx_channel->base.mem_block_num); //ALTER (cast)
    // set limit threshold, after transmit ping_pong_symbols size, an interrupt event would be generated
    rmt_ll_tx_set_limit((rmt_dev_t*)hal->regs, channel_id, tx_channel->ping_pong_symbols); //ALTER (cast)
    // disable carrier modulation by default, can reenable by `rmt_apply_carrier()`
    rmt_ll_tx_enable_carrier_modulation((rmt_dev_t*)hal->regs, channel_id, false); //ALTER (cast)
    // idle level is determined by register value
    rmt_ll_tx_fix_idle_level((rmt_dev_t*)hal->regs, channel_id, 0, true); //ALTER (cast)
    // always enable tx wrap, both DMA mode and ping-pong mode rely this feature
    rmt_ll_tx_enable_wrap((rmt_dev_t*)hal->regs, channel_id, true); //ALTER (cast)
    ...
    tx_channel->base.hw_mem_base = (rmt_symbol_word_t*)&RMTMEM.chan[channel_id + RMT_TX_CHANNEL_OFFSET_IN_GROUP].data32[0]; //ALTER &RMTMEM_n.channels[channel_id + RMT_TX_CHANNEL_OFFSET_IN_GROUP].symbols[0];
    ...
    *ret_chan = &tx_channel->base;
    //PRIu32 //ALTER
    ESP_LOGD(TAG, "new tx channel(%d,%d) at %p, gpio=%d, res=%dHz, hw_mem_base=%p, dma_mem_base=%p, ping_pong_size=%zu, queue_depth=%zu",
...
esp_err_t rmt_transmit(rmt_channel_handle_t channel, rmt_encoder_t *encoder, const void *payload, size_t payload_bytes, const rmt_transmit_config_t *config)
{
    ...
    portENTER_CRITICAL(&group->spinlock);
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id) | RMT_LL_EVENT_TX_LOOP_END(channel_id), true); //ALTER (cast)
    portEXIT_CRITICAL(&group->spinlock);
    return ESP_OK;
}
...
static void IRAM_ATTR rmt_tx_mark_eof(rmt_tx_channel_t *tx_chan)
{
    ...
    if (channel->dma_chan) {
        ...
        rmt_ll_enable_interrupt((rmt_dev_t*)group->hal.regs, RMT_LL_EVENT_TX_THRES(channel_id), false); //ALTER (cast)
...
static void IRAM_ATTR rmt_tx_do_transaction(rmt_tx_channel_t *tx_chan, rmt_tx_trans_desc_t *t)
{
    ...
    rmt_ll_tx_reset_pointer((rmt_dev_t*)hal->regs, channel_id); // reset pointer for new transaction //ALTER (cast)
    rmt_ll_tx_enable_loop((rmt_dev_t*)hal->regs, channel_id, t->loop_count != 0); //ALTER (cast)
    ...
    if (!channel->dma_chan) {
        ...
        rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_THRES(channel_id), t->loop_count == 0); //ALTER (cast)
        // Threshold interrupt will be generated by accident, clear it before starting new transmission
        rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_THRES(channel_id)); //ALTER (cast)
    }
    // don't generate trans done event for loop transmission
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id), t->loop_count == 0); //ALTER (cast)
    portEXIT_CRITICAL_ISR(&group->spinlock);
    ...
    portENTER_CRITICAL_ISR(&channel->spinlock);
    rmt_ll_tx_fix_idle_level((rmt_dev_t*)hal->regs, channel_id, t->flags.eot_level, true); //ALTER (cast)
    rmt_ll_tx_start((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
    portEXIT_CRITICAL_ISR(&channel->spinlock);
}
...
static esp_err_t rmt_tx_enable(rmt_channel_handle_t channel)
{
    ...
    rmt_ll_tx_reset_pointer((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
    rmt_ll_tx_enable_loop((rmt_dev_t*)hal->regs, channel_id, false); //ALTER (cast)
#if SOC_RMT_SUPPORT_TX_LOOP_COUNT
    rmt_ll_tx_reset_loop_count(hal->regs, channel_id);
    ...
    while (!(rmt_ll_tx_get_interrupt_status_raw((rmt_dev_t*)hal->regs, channel_id) & RMT_LL_EVENT_TX_DONE(channel_id))) {} //ALTER (cast)
#if SOC_RMT_SUPPORT_DMA
    if (channel->dma_chan) {
    ...
    portENTER_CRITICAL(&group->spinlock);
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id), true); //ALTER (cast)
    portEXIT_CRITICAL(&group->spinlock);
    return ESP_OK;
}
...
static esp_err_t rmt_tx_disable(rmt_channel_handle_t channel)
{
    ...
    portENTER_CRITICAL(&channel->spinlock);
    rmt_ll_tx_enable_loop((rmt_dev_t*)hal->regs, channel->channel_id, false); //ALTER (cast)
    ...
    portENTER_CRITICAL(&group->spinlock);
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_MASK(channel_id), false); //ALTER (cast)
#if !SOC_RMT_SUPPORT_TX_ASYNC_STOP
    ...
    while (!(rmt_ll_tx_get_interrupt_status_raw((rmt_dev_t*)hal->regs, channel_id) & RMT_LL_EVENT_TX_DONE(channel_id))) {} //ALTER (cast)
#endif
    rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_MASK(channel_id)); //ALTER (cast)
    portEXIT_CRITICAL(&group->spinlock);
...
static esp_err_t rmt_tx_modulate_carrier(rmt_channel_handle_t channel, const rmt_carrier_config_t *config)
{
    ...
    if (config && config->frequency_hz) {
        ...
        portENTER_CRITICAL(&channel->spinlock);
        rmt_ll_tx_set_carrier_level((rmt_dev_t*)hal->regs, channel_id, !config->flags.polarity_active_low); //ALTER (cast)
        rmt_ll_tx_set_carrier_high_low_ticks((rmt_dev_t*)hal->regs, channel_id, high_ticks, low_ticks); //ALTER (cast)
    ...
    portENTER_CRITICAL(&channel->spinlock);
    rmt_ll_tx_enable_carrier_modulation((rmt_dev_t*)hal->regs, channel_id, real_frequency > 0); //ALTER (cast)
    portEXIT_CRITICAL(&channel->spinlock);

    //ALTER PRIu32
    if (real_frequency > 0) {
        ESP_LOGD(TAG, "enable carrier modulation for channel(%d,%d), freq=%d Hz", group_id, channel_id, real_frequency);
    } else {
...
static bool IRAM_ATTR rmt_isr_handle_tx_done(rmt_tx_channel_t *tx_chan)
{
    ...
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id), false); //ALTER (cast)
    portEXIT_CRITICAL_ISR(&group->spinlock);
    ...
    if (trans_desc) {
        ...
        if (trans_desc->loop_count != 0) {
            rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id)); //ALTER (cast)
            return need_yield;
        }
    ...
    if (xQueueReceiveFromISR(tx_chan->trans_queues[RMT_TX_QUEUE_PROGRESS], &trans_desc, &awoken) == pdTRUE) {
        ...
        rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_DONE(channel_id), trans_desc->loop_count == 0); //ALTER (cast)
        rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_LOOP_END(channel_id), trans_desc->loop_count > 0); //ALTER (cast)
        portEXIT_CRITICAL_ISR(&group->spinlock);
...
static bool IRAM_ATTR rmt_isr_handle_tx_loop_end(rmt_tx_channel_t *tx_chan)
{
    ...
    if (trans_desc) {
        ...
        rmt_ll_tx_stop((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
        portEXIT_CRITICAL_ISR(&channel->spinlock);
#endif // SOC_RMT_SUPPORT_TX_LOOP_AUTO_STOP
        ...
        if (trans_desc->remain_loop_count) {
            ...
            rmt_ll_clear_interrupt_status((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_LOOP_END(channel_id)); //ALTER (cast)
            portENTER_CRITICAL_ISR(&channel->spinlock);
            rmt_ll_tx_set_loop_count((rmt_dev_t*)hal->regs, channel_id, this_loop_count); //ALTER (cast)
            rmt_ll_tx_reset_pointer((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
    ...
    portENTER_CRITICAL_ISR(&group->spinlock);
    // disable interrupt temporarily, re-enable it when there is transaction unhandled in the queue
    rmt_ll_enable_interrupt((rmt_dev_t*)hal->regs, RMT_LL_EVENT_TX_LOOP_END(channel_id), false); //ALTER (cast)
    portEXIT_CRITICAL_ISR(&group->spinlock);
...
static void IRAM_ATTR rmt_tx_default_isr(void *args)
{
    ...
    uint32_t status = rmt_ll_tx_get_interrupt_status((rmt_dev_t*)hal->regs, channel_id); //ALTER (cast)
